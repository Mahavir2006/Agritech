...
export default function BuyerDashboard() {
  ...
  const fetchOrders = async (userId: number) => {
    try {
      const response = await fetch(
        `/api/orders?userId=${userId}&userType=buyer`,
      );
      const data = await response.json();
  ...
  const fetchReceivedRatings = async (userId: number) => {
    try {
      const response = await fetch(
        `/api/ratings?userId=${userId}&userType=buyer`,
      );
      const data = await response.json();
  ...
  const fetchUserStats = async (userId: number) => {
    ...
    try {
      const response = await fetch(
        `/api/user-stats?userId=${userId}&userType=buyer`,
      );
      const data = await response.json();
      setUserStats(data);
    } catch (error) {
  ...
  const addToCart = async (productId: number, quantity: number = 1) => {
    ...
    try {
      ...
      if (result.success) {
        ...
        showAlert(
          result.error || result.message || "Failed to add to cart",
          "error",
        );
      }
    } catch (error) {
