...
} from "../lib/services/image-analysis-service";
...
interface ProductDetailsProps {
  ...
  userRole?: "farmer" | "buyer" | "wholesaler";
...
export default function ProductDetails({
  ...
  const [analysisData, setAnalysisData] = useState<PredictionResponse | null>(
    null,
  );
  ...
  const fetchSellerRating = async (sellerId: number) => {
    ...
    try {
      const response = await fetch(
        `/api/user-stats?userId=${sellerId}&userType=seller`,
      );
  ...
  useEffect(() => {
    const analyzeCurrentImage = async () => {
      ...
      if (
        ...
        currentPhoto.includes("placehold.co") ||
      ...
      try {
        const file = await urlToFile(
          currentPhoto,
          `product-image-${currentImageIndex}.jpg`,
        );
        ...
        const errorMessage =
          error instanceof Error
            ? error.message
            : "Something went wrong with the image analysis";
        setAnalysisError(errorMessage);
  ...
  const bulkDiscount =
    product.price_single > product.price_multiple
      ? Math.round(
          ((product.price_single - product.price_multiple) /
  ...
  const isOwnProduct =
    userRole === "farmer" && currentUserId === product.seller_id;
          ...
          <div>
            ...
            {isOwnProduct ? (
              <p className="text-sm text-green-600 mt-1">
                📋 Your Product - This is how buyers see your listing
              </p>
            ) : (
              <p className="text-sm text-blue-600 mt-1">
                🛒 Product Details - Add to cart or contact seller
              </p>
            )}
          </div>
            ...
            <div className="bg-gradient-to-r from-blue-50 to-purple-50 border border-blue-200 rounded-lg p-4">
              ...
              {analysisError && (
                <div className="flex items-start space-x-2 text-red-600 bg-red-50 border border-red-200 rounded-lg p-3">
                  ...
                  <div className="text-sm flex-1">
                    ...
                    {analysisError.includes("connect") ||
                    analysisError.includes("unavailable") ? (
                      <div className="text-xs text-red-400 mt-2">
                        The ML analysis service may be temporarily offline.
                        Product quality can still be assessed manually.
                      </div>
                    ) : null}
                    <button
              ...
              {analysisData && !isAnalyzing && (
                <div className="space-y-3">
                  ...
                  <div className="flex items-center justify-between">
                    ...
                    <span
                      className={`text-sm font-semibold ${getConfidenceColor(analysisData.confidence_score)}`}
                    >
                  ...
                  <div className="flex items-center justify-between">
                    ...
                    <div
                      className={`px-2 py-1 rounded border text-sm font-medium ${getQualityColor(analysisData.quality_score)}`}
                    >
                      {typeof analysisData.quality_score === "number"
                        ? `${analysisData.quality_score}/5`
                        : "N/A"}
                      {analysisData.quality_score >= 4 && (
                  ...
                  {typeof analysisData.quality_score === "number" &&
                    !isNaN(analysisData.quality_score) && (
                        ...
                        <div className="w-full bg-gray-200 rounded-full h-2">
                          <div
                            className={`h-2 rounded-full transition-all duration-300 ${
                              analysisData.quality_score >= 4
                                ? "bg-green-500"
                                : analysisData.quality_score >= 3
                                  ? "bg-yellow-500"
                                  : "bg-red-500"
                            }`}
                            style={{
                              width: `${Math.max(0, Math.min(100, (analysisData.quality_score / 5) * 100))}%`,
                            }}
                          ></div>
                        </div>
                      </div>
              ...
              {!analysisData && !isAnalyzing && !analysisError && (
                <div className="text-sm text-gray-700 text-center py-2">
                  {photos[currentImageIndex]?.includes("placeholder") ||
                ...
                <div className="flex items-center space-x-2">
                  ...
                  <div>
                    ...
                    <p className="text-sm text-green-700 mt-1">
                      This is how buyers see your product listing. You can edit
                      it from your "My Crops" section.
                    </p>
                  </div>
                </div>
              </div>
