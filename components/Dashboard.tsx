...
export default function Dashboard({
  ...
  const fetchWeather = async () => {
    try {
      ...
      if (userLocation) {
        ...
        console.log(
          "Fetching weather for coordinates:",
  ...
  const avgPrice =
    products.length > 0
      ? products.reduce((sum, p) => sum + (p.price_single || 0), 0) /
        products.length
      : 0;
  ...
  const monthlyTarget =
    userType === "farmer" ? 50000 : userType === "wholesaler" ? 100000 : 25000; // Wholesaler has higher target
  ...
  return (
    <div className="space-y-6">
      {/* Welcome Header */}
      <div
        className={`${userType === "wholesaler" ? "bg-violet-600" : "bg-green-600"} rounded-2xl p-6 text-white`}
      >
        <div className="flex items-center justify-between">
          <div>
            ...
            <p
              className={`${userType === "wholesaler" ? "text-violet-100" : "text-green-100"}`}
            >
              {userType === "farmer"
                ? "Ready to manage your farm and connect with buyers?"
                : userType === "wholesaler"
                  ? "Manage bulk orders and connect with farmers efficiently."
                  : "Ready to discover fresh produce from local farmers?"}
            </p>
          </div>
          <div className="text-right">
            <div
              className={`text-sm ${userType === "wholesaler" ? "text-violet-100" : "text-green-100"}`}
            >
            ...
            <div>
              ...
              {weather.isDemo && (
                <p className="text-xs text-blue-200 mt-2 opacity-75">
                  * Demo data - Please check your OpenWeather API key
                  configuration
                </p>
              )}
            </div>
            ...
            <div className="space-y-4">
              <div>
                ...
                <p className="text-xs text-gray-500 mt-1">
                  {Math.round(revenueProgress)}% of monthly target
                </p>
              </div>
